        ...
        <div className="space-y-2">
          ...
          <div className="space-y-1.5">
            <h1 className="text-2xl font-semibold tracking-tight text-foreground lg:text-[26px]">
              {viewMode === 'dashboard' ? t.tracker.portfolioDashboardTitle : t.tracker.canvasWorkspaceTitle}
            </h1>
            <p className="max-w-3xl text-sm text-muted-foreground">
              {viewMode === 'dashboard'
                ? t.tracker.portfolioDashboardSubtitle
                : t.tracker.canvasWorkspaceSubtitle
              }
            </p>
          </div>
        ...
        <div className="flex flex-col gap-2 rounded-xl bg-card/90 px-3 py-3 text-xs text-muted-foreground ring-1 ring-border/70 backdrop-blur lg:w-[280px]">
          <div className="flex items-center justify-between gap-2 text-[11px]">
            ...
            <div className="inline-flex items-center gap-2 rounded-full bg-background/80 px-3 py-1 ring-1 ring-border/60">
              {loading && <span className="h-2 w-2 animate-pulse rounded-full bg-primary" aria-hidden="true" />}<span>{loading ? t.tracker.loadingData : t.tracker.lastSynced}</span>
            </div>
          </div>
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-[13px] text-foreground">{t.tracker.portfolioLive}</p>
                <p className="text-[11px] text-muted-foreground">{t.tracker.autosaveActive}</p>
              </div>
            </div>
            <Button
              variant="outline"
